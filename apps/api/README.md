...
`messages` a été retiré du plan (remplacé par l'appel, voir `docs/decisions.md`) — la route reste en `501` en attendant d'être supprimée ou réaffectée. Paramètres de la plateforme (zones, métiers, commission, textes légaux) pas encore exposés côté admin (l'écran existe dans `apps/admin` mais reste en attente) — le stockage clé-valeur existe déjà (`ParametrePlateforme`, voir `src/lib/parametres.ts`), il manque l'écran d'édition.
