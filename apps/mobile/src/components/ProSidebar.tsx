...
// Tiroir de navigation dedie au role professionnel (regroupe les ecrans
// jusque-la eparpilles en icones sur Profil), construit a la main comme
// FloatingTabBar plutot que d'ajouter une dependance de navigation par tiroir.
export default function ProSidebar({ visible, onClose }: ProSidebarProps) {
  ...
  function allerA(href: (typeof LIENS)[number]["href"]) {
    // Naviguer avant de fermer : l'ecran hote (index) peut etre gele par
      ...
      <Animated.View style={[styles.panneau, { transform: [{ translateX }] }]}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.sousTitre}>Espace professionnel</Text>
          </View>
        ...
        <View style={styles.liste}>
          {LIENS.map(({ href, label, icone: Icone }) => (
            <Pressable key={href} style={styles.item} onPress={() => allerA(href)}>
